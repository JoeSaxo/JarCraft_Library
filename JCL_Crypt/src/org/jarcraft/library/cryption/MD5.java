...
public class MD5 {
	...
	public static void main(String[] args) {
		...
		String username = "1040840604414249bc9370a67959a992";
		System.out.println(crypt(username));

		System.exit(0);
		String tempString, cryptedString3;
		tempString = "temp";
		cryptedString3 = crypt(tempString, 100);
		System.out.println(tempString);
		System.out.println(cryptedString3);
		System.out.println(equals(cryptedString3, crypt(crypt(crypt(tempString))), 97));
		System.out.println(equals(cryptedString3, "temp"));
		System.out.println(equals(cryptedString3, "temp", 100));
		String temp = tempString;
		StopWatch stop = new StopWatch();
		stop.start();
		temp = encrypt(4, crypt(tempString), 4, "0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ");
		stop.stop();
		System.out.println(stop.getTime() + " | " + temp);
		/*
		while ((temp = crypt(temp)) != null) {
	...
	public static String crypt(String data, int factor) {
		if (factor < 1) return data;
		try {
			byte[] bytes = data.getBytes("ISO-8859-1");
			MessageDigest md5er = MessageDigest.getInstance("MD5");
			byte[] hash = md5er.digest(bytes);
			return crypt(bytes2hex(hash), factor - 1);
	...
	public static boolean equals(String cryptedData, String uncryptedData, int factor) {
		if (factor < 1) return cryptedData.equals(uncryptedData);
		return equals(cryptedData, crypt(uncryptedData), factor - 1);
	}
	
	public static boolean equals(String cryptedData, String uncryptedData) {
		return equals(cryptedData, uncryptedData, 1);
	}
